...
def upload_to_firestore(df_new):
    ...
    for _, row in df_upload.iterrows():
        ...
        if count % 200 == 0:
            ...
            time.sleep(0.05)
    batch.commit()
    bar.empty(); msg.success(f"✅ {total}건 업데이트 완료!")
...
with tabs[0]: # 매출
    st.subheader("매출 페이스 분석")
    def get_pace(d): return d.groupby('LeadTime')['총금액'].sum().sort_index(ascending=False).cumsum().sort_index() if not d.empty else pd.Series()
    pt, pr = get_pace(target_df), get_pace(ref_df)
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=pt.index, y=pt.values, name='Target', line=dict(color='#0052cc')))
    if not pr.empty: fig.add_trace(go.Scatter(x=pr.index, y=pr.values, name='Ref', line=dict(color='gray', dash='dot')))
    fig.update_layout(xaxis={'autorange':'reversed'}, title="누적 매출 추이")
    st.plotly_chart(fig, use_container_width=True)

with tabs[5]: # 로열티 (에러 수정판)
    ...
    if not f_n: st.warning("고객명 컬럼 없음")
        ...
        def segment(n):
            ...
            else: return "3회 이상 (VIP)"
        g_stats['Grade'] = g_stats['TotalVisits'].apply(segment)
        t_l = pd.merge(df_l[df_l['예약번호'].isin(target_df['예약번호'])], g_stats[['GuestKey','Grade']], on='GuestKey', how='left')
        
        m1, m2, m3 = st.columns(3)
        u_g = t_l['GuestKey'].nunique(); r_g = t_l[t_l['Grade'] != "1회 (신규)"]['GuestKey'].nunique()
...
with tabs[6]: # RM 정밀 분석
    st.header("🚀 수익 관리(RM) 기간별 분석")
    rm_mode = st.radio("분석 기준", ["입실일자", "예약일자"], horizontal=True, key="rm_mode_radio")
    date_col = '입실일자' if "입실일자" in rm_mode else '예약일자'
    d_a = st.date_input("기준 기간 (A)", [datetime(2025,1,1), datetime(2025,1,31)], key="da_rm")
    d_b = st.date_input("비교 기간 (B)", [datetime(2026,1,1), datetime(2026,1,24)], key="db_rm")
    df_a = df_raw[(df_raw[date_col].dt.date >= d_a[0]) & (df_raw[date_col].dt.date <= d_a[1])]
    df_b = df_raw[(df_raw[date_col].dt.date >= d_b[0]) & (df_raw[date_col].dt.date <= d_b[1])]
    if not df_a.empty and not df_b.empty:
        st.metric("매출 변동", f"{df_b['총금액'].sum()/10000:,.0f}만", f"{(df_b['총금액'].sum()-df_a['총금액'].sum())/10000:,.0f}만")
        df_b['is_cancel'] = df_b['상태'].isin(def_exc)
        cxl = df_b.groupby('거래처')['is_cancel'].mean() * 100
        st.plotly_chart(px.bar(cxl.reset_index(), x='거래처', y='is_cancel', title="거래처별 취소율(%)"), use_container_width=True)

with tabs[7]: # 수익 전략 (에러 수정 포인트: trendline 제거)
    st.header("🎯 수익 극대화 전략")
    s_tabs = st.tabs(["💰 황금 ADR", "🌍 국적 분석", "🛡️ 취소 예측"])
    with s_tabs[0]:
        df_g = df_clean.groupby('입실일자').agg({'총금액':'sum','객실수':'sum'}).reset_index()
        df_g['ADR'] = df_g['총금액']/df_g['객실수']
        # statsmodels 에러 방지를 위해 trendline="lowess" 제거
        st.plotly_chart(px.scatter(df_g, x='ADR', y='총금액', size='객실수', title="가격 대비 매출 상관관계 (점이 높은 곳이 최적가)"), use_container_width=True)
    with s_tabs[1]:
...
# -----------------------------------------------------------------------------
try:
    st.session_state["historical_dow"] = (df_raw['예약일자'].dt.dayofweek.value_counts(normalize=True)*7).to_dict()
...
st.divider()
with st.expander("Raw Data 확인"): st.dataframe(df_view.head(50))
