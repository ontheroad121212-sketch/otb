...
# ==============================================================================
# 4. 엑셀/CSV 파일 처리 및 매핑 로직 (OTB 로직 및 조식 로직 강화)
# ==============================================================================
...
def process_data(file, status, force_otb=False):
    """
    [파일 분석 결과 반영 수정본]
    1. 헤더 자동 찾기 기능 강화 (키워드 매칭)
    2. OTB: 물리적 마지막 열을 무조건 매출로 인식
    3. 조식: Service_Code에 'BF'가 포함되어 있는지 확인
    """
    try:
        ...
        # 헤더 자동 찾기 로직 강화
        # (파일 상단에 불필요한 정보가 많으므로 실제 헤더 행을 찾아야 함)
        keywords = ['guest', 'name', 'check', 'date', 'room', '고객', '입실', '객실', '서비스', '매출', '일자']
        best_row = 0; max_hit = 0
        for i, row in df_raw.head(20).iterrows(): # 상위 20행 검사
        ...
        if is_otb:
            ...
            # [수정] OTB 매출은 테이블의 물리적 '가장 마지막 열'을 무조건 가져옴
            # 엑셀 파일에서 가장 오른쪽 끝에 있는 데이터가 매출 합계임
            try:
                # 마지막 열 가져오기 (Empty column 제외하고 값이 있는 마지막 열)
                # 역순으로 탐색하여 첫번째로 값이 있는 열을 찾음
                rev_col_idx = -1
                
                # 강제로 마지막 열 지정 (iloc 사용)
                df['Room_Revenue'] = pd.to_numeric(
                    df_raw.iloc[:, rev_col_idx].astype(str).str.replace(',', '').str.replace('₩', '').str.replace('nan', '0'),
                    errors='coerce'
                ).fillna(0)
                
                # 객실수(RN) 찾기: 보통 매출 열보다 앞쪽에 위치
                # '객실수' 혹은 'RN' 이라는 이름이 포함된 마지막 열 사용
                rn_candidates = [c for c in df_raw.columns if any(k in str(c).upper() for k in ['RN', '객실수', 'QTY', 'ROOMS'])]
                if rn_candidates:
                    # 후보 중 가장 뒤에 있는 것 (보통 합계 객실수)
                    df['RN'] = pd.to_numeric(df_raw[rn_candidates[-1]], errors='coerce').fillna(0)
                else:
                    # 없으면 뒤에서 5번째 열 (경험적 추정)
            ...
            df['Guest_Name'] = 'OTB_DATA'; df['Segment'] = 'OTB'; df['Account'] = 'OTB'
        ...
        # ---------------------------------------------------------
            ...
            def check_bf(row):
                ...
                # 사용자가 지정한 BF 코드들 (BF, BF2P, BF4P, BFAD, BFUP, BFCH 등) 모두 'BF'를 포함함
                if 'BF' in svc_code:
            ...
            # 숫자 변환 (clean_numeric_columns에서 처리되지만 여기서 미리 처리하여 RN 계산)
