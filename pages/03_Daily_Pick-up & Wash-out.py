...
def process_data(uploaded_file, status, force_otb=False):
    ...
    try:
        ...
        # ---------------------------------------------------------
        if is_otb:
            # 1. 파일 상단에서 날짜 찾기 (파일명 대신 파일 내용에서 월(Month) 추출)
            target_month_date = None
            date_pattern = re.compile(r'20\d{2}-\d{2}') # YYYY-MM 패턴 찾기
            ...
            # 2. 총 매출 찾기 (파일의 가장 마지막 행, 가장 마지막 열이 합계임)
            # 빈 행/열 제거 후 마지막 값 추출
            ...
            try:
                ...
                clean_val = raw_val.replace(',', '').replace('nan', '0').split('.')[0] # 소수점 제거
                total_rev = int(clean_val)
            except:
                total_rev = 0
            
            # 3. 데이터프레임 생성 (월별 1개 행으로 요약)
            df = pd.DataFrame([{
                'CheckIn': target_month_date.strftime('%Y-%m-%d'),
                'Room_Revenue': total_rev,
                'Total_Revenue': total_rev,
                'RN': 0, # OTB 파일에서 RN 합계까지 찾기 복잡하면 매출 우선
                'Guest_Name': 'OTB_DATA',
                'Segment': 'OTB',
                'Account': 'OTB_Summary',
                'Room_Type': 'ROH',
        ...
        # ---------------------------------------------------------
            ...
            # [조식 식별 로직] Service_Code(K열)에 BF가 있는지 확인
            def check_breakfast(row):
                # Service_Code가 가장 확실함. 없으면 다른 컬럼도 참조
                svc_code = str(row.get('Service_Code', '')).upper()
                rate_plan = str(row.get('Rate_Plan', '')).upper()
                package = str(row.get('Package', '')).upper()
                
                if 'BF' in svc_code or '조식' in svc_code or 'BREAKFAST' in svc_code or 'PKG' in svc_code:
                    return 'Included (조식포함)'
                if '조식' in rate_plan or 'PKG' in rate_plan:
                    return 'Included (조식포함)'
...
def render_analysis_tab(target_df, title_prefix, unique_key, color_scale="Blues"):
    ...
    with t8:
        ...
        if 'Breakfast' in target_df.columns:
            ...
            c2.plotly_chart(px.bar(bf_stats, x='Breakfast', y='Room_Revenue', title="조식 포함 여부별 매출"), use_container_width=True, key=f"{unique_key}_bf_bar")
