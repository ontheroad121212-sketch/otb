...
def process_data(uploaded_file, status, force_otb=False):
    try:
        ...
        # ---------------------------------------------------------
        # Case B: 예약/취소 리스트 (CSV cp949 인코딩 & 리드타임 핀셋 추출)
        # ---------------------------------------------------------
        else:
            # 1. 파일 읽기 - 한글 헤더 깨짐 방지를 위해 cp949 인코딩 사용
            if uploaded_file.name.endswith('.csv'):
                try: df_raw = pd.read_csv(uploaded_file, header=2, encoding='cp949')
                except: df_raw = pd.read_csv(uploaded_file, header=2) # fallback
            else:
                df_raw = pd.read_excel(uploaded_file, header=2)

            # 2. [핵심] 리드타임 열을 직접 찾아서 데이터 확보 (인덱스 기준 아님, 이름 기준)
            # 인코딩이 맞다면 '리드타임' 글자가 정상적으로 보입니다.
