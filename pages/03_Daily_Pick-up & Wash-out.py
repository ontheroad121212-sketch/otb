...
# ==============================================================================
# 4. 엑셀 처리 로직 (K열 BF 강제 지정 및 OTB 마지막 셀 추출)
# ==============================================================================
...
def process_data(file, status, force_otb=False):
    try:
        ...
        # ---------------------------------------------------------
        # [A] OTB 처리: 사용자 요청대로 무조건 마지막 행, 마지막 열
        # ---------------------------------------------------------
        if is_otb:
        ...
        if header_idx != -1:
            ...
            # [핵심] 조식 여부: K열(인덱스 10)에 'BF'가 있는지 검사
            def check_bf_direct(row):
                try:
                    # K열(물리적 11번째) 값을 문자열로 가져와서 BF 포함 확인
                    k_val = str(row.iloc[10]).upper() if len(row) > 10 else ""
                    return 'Included' if 'BF' in k_val else 'Room Only'
                except: return 'Room Only'
            
            # 조식 정보를 리스트로 미리 계산
            bf_list = [check_bf_direct(row) for _, row in df_data.iterrows()]
            
            # 이제 헤더를 입히고 일반 컬럼 매핑 진행
...
def render_analysis_tab(target_df, title_prefix, unique_key):
    ...
    with t2:
        ...
        st.plotly_chart(px.imshow(piv, text_auto="d", aspect="auto", title="Booking Pacing"), use_container_width=True, key=f"{unique_key}_pacing")
    ...
    with t4:
        ...
        st.plotly_chart(px.bar(wd_stats, x='Day_Type', y='Room_Revenue', title="주말/평일 매출"), use_container_width=True, key=f"{unique_key}_day")
        show_dataframe_with_style(add_total_row(wd_stats, 'Day_Type'))

    with t5:
        # [수정] 조식 탭: Breakfast 컬럼 기반 출력
        st.subheader("🍳 조식 포함 여부 분석 (K열 BF 기준)")
...
try:
    ...
    with st.sidebar:
        ...
        if st.button("🗑️ OTB 초기화"):
            ...
            st.warning(f"OTB 데이터 {cnt}건 삭제됨."); time.sleep(1); st.cache_data.clear(); st.rerun()
        ...
        with st.expander("데이터 업로드", expanded=True):
            ...
            f3_list = st.file_uploader("OTB (12개월)", type=['xlsx','csv'], key="f3", accept_multiple_files=True)
            if f3_list and st.button("OTB 저장"):
                ...
                if otb_list:
                    if save_to_firestore(pd.concat(otb_list, ignore_index=True)):
                        st.success("OTB 통합 저장 성공!"); st.cache_data.clear(); st.rerun()
    ...
    if selected_date and not df_all.empty:
        ...
        with main_tabs[1]: render_analysis_tab(df_paid_bk, "예약", "bk_u")
        with main_tabs[2]: render_analysis_tab(df_list_cn, "취소", "cn_u")
        with main_tabs[3]: render_analysis_tab(df_total_paid, "종합", "tot_u")
        with main_tabs[4]: 
        ...
        with main_tabs[5]:
            ...
            if df_otb.empty: st.warning("OTB 데이터 없음")
                ...
                st.plotly_chart(fig, use_container_width=True, key="otb_final_chart")
