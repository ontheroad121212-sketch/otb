...
import numpy as np
import time
import openpyxl # 엑셀 색상 인식을 위해 추가
...
def clean_numeric_columns(df):
    ...
    for col in target_cols:
        if col in df.columns:
            # 문자열 변환 -> 통화기호/콤마 제거 -> nan 제거 -> 숫자 변환
...
# ==============================================================================
# 4. 엑셀/CSV 파일 처리 및 매핑 로직 (조식 색상 & OTB 강화)
# ==============================================================================
...
def check_yellow_color(file_obj, header_row_idx):
    """
    [핵심] 엑셀 파일에서 '노란색' 배경이 칠해진 행의 인덱스를 찾아냅니다.
    """
    yellow_indices = set()
    try:
        file_obj.seek(0)
        ...
        # openpyxl은 1-based index, pandas header_row_idx는 0-based
        # 데이터 시작 행은 header_row_idx + 2 (1 for 0-index match, 1 for next row)
        ...
        for i, row in enumerate(ws.iter_rows(min_row=start_row), start=0):
            ...
            for cell in row:
                if cell.fill and cell.fill.fgColor:
                    # 노란색 계열 색상 코드 확인 (FFFF00 or Similar)
                    # rgb 값이 없는 경우(Theme color)는 제외하고 RGB가 있는 경우만 체크
                    color = cell.fill.fgColor.rgb
                    if color and (str(color).upper().startswith('FFFF00') or str(color).upper() == 'FFFFFF00'):
                        is_yellow = True
                        break
            if is_yellow:
                yellow_indices.add(i)
                
    except Exception as e:
        print(f"Color check failed: {e}")
        
    file_obj.seek(0) # 파일 포인터 원복
    return yellow_indices

def process_data(file, status, force_otb=False):
    """
    [파일 분석 및 데이터 전처리]
    - OTB: 물리적 마지막 열을 매출로 인식 (통화기호 제거)
    - 조식: '노란색' 셀 색상 인식 + 텍스트(BF) 검색 병행
    """
    try:
        ...
        if is_otb:
            ...
            # 빈 열(Empty Column) 제거 (엑셀 서식 때문에 생긴 빈 열들)
            ...
            # [수정] OTB 매출: 유효한 컬럼 중 '가장 마지막 열' 선택
            # 통화(Currency) 서식 때문에 문자로 인식될 수 있으므로 clean_numeric_columns가 처리하도록 함
            try:
                # 마지막 열 가져오기
                last_col_name = df_raw.columns[-1]
                
                # 만약 마지막 열이 '비고' 같은 것이면, '매출'이나 'Revenue'가 들어간 마지막 열을 찾음
                rev_candidates = [c for c in df_raw.columns if '매출' in str(c) or 'Rev' in str(c) or 'Amount' in str(c)]
                
                if rev_candidates:
                    target_col = rev_candidates[-1] # 가장 뒤에 있는 매출 컬럼
                else:
                    target_col = last_col_name # 없으면 그냥 맨 마지막 열
                
                df['Room_Revenue'] = df_raw[target_col] # 나중에 숫자 변환
                ...
                if rn_candidates:
                    df['RN'] = df_raw[rn_candidates[-1]]
                else:
                    df['RN'] = df_raw.iloc[:, -5] # fallback
            except Exception as e:
                df['Room_Revenue'] = 0
                df['Total_Revenue'] = 0
                df['RN'] = 0
        ...
        # ---------------------------------------------------------
            ...
            if file.name.endswith('.xlsx'):
                yellow_rows = check_yellow_color(file, best_row)
            
            # 2. 텍스트 감지 및 통합
            def check_breakfast_final(row):
                # A. 노란색 체크 (인덱스 기반)
                if row.name in yellow_rows:
                    return 'Included (Yellow)'
                
                # B. 텍스트 체크 (서비스코드, 상품명 등)
                text = (str(row.get('Service_Code', '')) + " " + 
                        str(row.get('Rate_Plan', '')) + " " + 
                        str(row.get('Room_Type', '')) + " " + 
                        str(row.get('Segment', ''))).upper()
                
                if any(k in text for k in ['BF', '조식', 'BREAKFAST', 'BKFST', 'PKG', 'INCL']):
                    return 'Included (Text)'
                
                return 'Not Included'
            
            # 인덱스를 유지하며 적용
            df['Breakfast'] = df.apply(check_breakfast_final, axis=1)
            ...
            # 숫자 변환은 clean_numeric_columns에서 일괄 처리
        ...
        # [중요] 숫자 컬럼 클렌징 (통화 기호 등 제거)
        return clean_numeric_columns(df)

    except Exception as e:
        st.error(f"처리 중 오류 ({file.name}): {e}")
        return pd.DataFrame()
...
def render_tab(df, key_prefix):
    ...
    with t8:
        if 'Breakfast' in df.columns:
            st.subheader("🍳 조식 분석 현황 (노란색 셀 포함)")
        ...
        with st.expander("예약 리스트", expanded=True):
            ...
            if f1 and st.button("예약 저장"):
                ...
                if not df.empty and save_to_firestore(df):
                    st.success("예약 리스트 저장 성공! (노란색 셀 인식 완료)")
                    time.sleep(1)
                    st.cache_data.clear()
                    st.rerun()
