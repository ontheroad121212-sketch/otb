...
def render_tab(df, k):
    ...
    with t2:
        ...
        if not p.empty:
            fig_hm = go.Figure(data=go.Heatmap(z=p.values, x=p.columns, y=p.index, colorscale='Blues', text=p.values, texttemplate="%{text:.0f}"))
            fig_hm.update_layout(xaxis={'type':'category', 'title':T("투숙월 (Stay)")}, yaxis={'type':'category', 'title':T("예약생성월 (Booking)")})
            st.plotly_chart(fig_hm, use_container_width=True, key=f"{k}_hm")
        else: st.warning(T("데이터 없음"))
    with t3: group_and_show(df, 'Account')
    with t4:
        df['LT_G'] = pd.cut(df['RN'], bins=[-999,0,3,7,14,30,60,90,999], labels=['0','1-3','4-7','8-14','15-30','31-60','61-90','90+'])
...
try:
    ...
    with st.sidebar:
        ...
        dates_selected = st.date_input(T("기준 기간 (어제까지 선택 가능)"), value=(yesterday, yesterday), max_value=yesterday)
        comp_selected = st.date_input(T("비교 기간 (선택사항)"), value=(), max_value=yesterday)
    ...
    # 데이터 필터링
    mask = (df_all['Snapshot_Date'] >= sel_start) & (df_all['Snapshot_Date'] <= sel_end)
    df_bk = df_all[mask & (df_all['Data_Type']=='Reservation') & (df_all['Total_Revenue']>0)].copy()
    df_cn = df_all[mask & (df_all['Data_Type']=='Cancellation')].copy()
    if df_cn.empty: df_cn = df_all[mask & (df_all['Status']=='RC')].copy()
    df_zero = df_all[mask & (df_all['Data_Type']=='Reservation') & (df_all['Total_Revenue']<=0)].copy()
    df_tot = pd.concat([df_bk, df_cn])

    # OTB 데이터 정의 (누락된 부분 추가)
    ...
    with tabs[0]:
        ...
        if df_bk.empty and df_cn.empty: st.info(T("데이터 없음"))
            ...
            col = st.columns(5)
            col[0].metric(T("RN"), f"{b_rn:,.0f}")
            col[1].metric(T("객실료"), f"{b_rev:,.0f}")
            col[2].metric(T("총매출"), f"{b_tot:,.0f}")
            col[3].metric(T("ADR(Room)"), f"{b_rev/b_rn if b_rn>0 else 0:,.0f}")
            col[4].metric(T("건수"), f"{len(df_bk):,.0f}")
            
            st.markdown(f"#### {T('❌ 취소 (Cancellation List)')}")
            col = st.columns(5)
            col[0].metric(T("RN"), f"{c_rn:,.0f}")
            col[1].metric(T("객실료"), f"{c_rev:,.0f}")
            col[2].metric(T("총매출"), f"{c_tot:,.0f}")
            col[3].metric(T("ADR(Room)"), f"{c_rev/c_rn if c_rn>0 else 0:,.0f}")
            col[4].metric(T("건수"), f"{len(df_cn):,.0f}")
