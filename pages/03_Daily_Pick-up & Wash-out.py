...
# ==============================================================================
# 1. 페이지 설정 및 CSS 스타일링 (원본 스타일 100% 복구)
# ==============================================================================
st.set_page_config(
...
# ==============================================================================
# 4. 엑셀/CSV 파일 처리 및 매핑 로직 (리드타임 수정)
# ==============================================================================
...
def normalize_and_map_columns(df):
    ...
    rules = {
        ...
        # Lead_Time은 별도 로직으로 처리하므로 여기서 제외하거나 보조로 둠
...
def process_data(uploaded_file, status, force_otb=False):
    try:
        ...
        # ---------------------------------------------------------
        # Case B: 예약/취소 리스트 처리 (3행 헤더 리드타임 추출)
        # ---------------------------------------------------------
        else:
            if uploaded_file.name.endswith('.csv'):
                try: df_raw = pd.read_csv(uploaded_file, header=None)
                ...
                # [수정] 엑셀 3행(Index 2)을 헤더로 강제 지정 (사용자 요청)
                df = pd.read_excel(uploaded_file, header=2)

            # [수정] 리드타임 열 강제 찾기 및 추출
            # 헤더 이름 중 '리드'나 'LT'나 'Lead'가 포함된 열을 찾음
            lt_col_name = None
            ...
            # 리드타임 데이터 별도 저장 (나중에 덮어씌움)
            if lt_col_name:
                lead_time_series = pd.to_numeric(df[lt_col_name].astype(str).str.replace(',', ''), errors='coerce').fillna(0)
            else:
                lead_time_series = 0
            ...
            df['Lead_Time'] = lead_time_series # 추출한 리드타임 적용
...
def render_analysis_tab(target_df, title_prefix, unique_key, color_scale="Blues"):
    ...
    with t6:
        ...
        c1, c2 = st.columns(2)
        fig_day_bar = px.bar(wd_stats, x='Day_Type', y='Room_Revenue')
        c1.plotly_chart(fig_day_bar, use_container_width=True, key=f"{unique_key}_day_bar")
        fig_day_pie = px.pie(wd_stats, values='RN', names='Day_Type')
        c2.plotly_chart(fig_day_pie, use_container_width=True, key=f"{unique_key}_day_pie")
