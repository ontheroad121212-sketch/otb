...
def process_data(file, status, force_otb=False):
    ...
    try:
        ...
        if is_otb:
            ...
            # [수정] 조식 식별: Service_Code에 'BF'가 포함되어 있으면 무조건 조식 포함
            def check_bf(row):
...
def add_total_row(df, group_col_name="구분"):
    ...
    row = {c: "" for c in df.columns}; row.update(total); row[group_col_name]="TOTAL"
...
# [수정] 함수 이름 통일 (show_dataframe_with_style로 변경)
def show_dataframe_with_style(df):
    ...
    # 예외처리: Budget_Achiev 컬럼 포맷
    if 'Budget_Achiev' in df.columns:
        fmt['Budget_Achiev'] = "{:.1f}%"
...
def render_analysis_tab(target_df, title_prefix, unique_key, color_scale="Blues"):
    ...
    with t4:
        ...
        df_lt['LG'] = pd.cut(df_lt['Lead_Time'], [-1,0,3,7,14,30,60,90,999], labels=['0','1-3','4-7','8-14','15-30','31-60','61-90','90+'])
        l = df_lt.groupby('LG').agg({'RN':'sum','Room_Revenue':'sum'}).reset_index()
