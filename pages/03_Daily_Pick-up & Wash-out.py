...
def process_data(file, status, force_otb=False):
    try:
        ...
        # [B] 예약/취소 리스트 (충돌 해결 핵심)
            ...
            # 4. 컬럼 매핑 (여기서 기존 리드타임 컬럼이 사라질 수 있음)
            df = normalize_columns(df_data).copy()
            
            # 5. [핵심] 아까 확보한 리드타임 값을 강제로 덮어씌움 (복구)
...
try:
    ...
    if sel_date and not df_db.empty:
        ...
        with tabs[0]:
            ...
            st.markdown("#### ✅ 신규 예약")
            c = st.columns(5)
            c[0].metric("RN", f"{b_rn:,.0f}"); c[1].metric("매출", f"{b_rev:,.0f}"); c[2].metric("ADR", f"{b_rev/b_rn if b_rn>0 else 0:,.0f}"); c[3].metric("LOS", f"{b_los:.1f}박"); c[4].metric("건수", f"{len(df_bk):,.0f}")
            
            st.markdown("#### ❌ 금일 취소")
            cc = st.columns(5)
            cc[0].metric("RN", f"{c_rn:,.0f}"); cc[1].metric("매출", f"{c_rev:,.0f}"); cc[2].metric("ADR", f"{c_rev/c_rn if c_rn>0 else 0:,.0f}"); cc[3].metric("LOS", f"{c_los:.1f}박"); cc[4].metric("건수", f"{len(df_cn):,.0f}")
            st.divider(); style_df(add_total(df_bk.groupby('Segment').agg({'RN':'sum','Room_Revenue':'sum'}).reset_index(), 'Segment'))
            
        with tabs[1]: render_tabs(df_bk, "booked")
        with tabs[2]: render_tabs(df_cn, "cancelled")
        with tabs[3]: render_tabs(pd.concat([df_bk, df_cn]), "total")
        with tabs[4]:
            if not df_otb.empty:
