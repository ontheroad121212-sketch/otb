    ...
    try:
        ...
        for doc in docs:
            ...
            if 'data' in doc_dict:
                ...
                # 구버전 데이터(Segment에 OTB가 있는 경우) 처리 호환성
                rows = doc_dict['data']
                if doc_data_type == 'Reservation' and len(rows) > 0 and 'OTB' in str(rows[0].get('Segment', '')):
                    doc_data_type = 'OTB'
...
# ==============================================================================
# 4. 파일 처리 로직 (RN 직접 계산 + 매핑 오인 방지 + 날짜분할)
# ==============================================================================
...
def normalize_and_map_columns(df):
    ...
    # [핵심] 뻥튀기 원인 차단: 제외 단어 설정
    exclusions = {
        'Rooms': ['revenue', 'rate', 'type', '금액', '료', '번호', 'id', 'pax', 'adult', 'child', 'person', 'no', 'code', 'phone'],
        'Nights': ['date', '일자', 'time', '시각', 'code', 'id', 'no']
    }

    for col in df.columns:
        clean = str(col).lower().replace(" ", "").replace("_", "")
        for target, kws in rules.items():
            # 제외 단어가 포함되어 있으면 절대 매핑하지 않음
...
def process_data(uploaded_file, status):
    try:
        ...
        # 6. Snapshot_Date 생성 (통데이터 분할의 기준)
        ...
        # 7. [핵심] 숫자 정리 및 RN 직접 계산 (뻥튀기 방지)
        # Rooms와 Nights를 숫자로 변환 (NaN은 0으로, Nights NaN은 1로)
        df['Rooms_Clean'] = pd.to_numeric(df.get('Rooms', 0), errors='coerce').fillna(0)
        
        # [Sanity Check] 객실수가 50개를 넘으면 1로 강제 보정 (예약번호 오인 방지)
        df['Rooms_Clean'] = np.where(df['Rooms_Clean'] > 50, 1, df['Rooms_Clean'])
        
        df['Nights_Clean'] = pd.to_numeric(df.get('Nights', 1), errors='coerce').fillna(1)
        # [Sanity Check] 박수가 365개를 넘으면 1로 강제 보정
        df['Nights_Clean'] = np.where(df['Nights_Clean'] > 365, 1, df['Nights_Clean'])
        
        # RN = 객실수 * 박수 (사용자 요청: 너가 계산해)
...
try:
    ...
    with tabs[0]:
        ...
        if df_paid_bk.empty and df_list_cn.empty:
            ...
            # LOS 계산 (제로 디비전 방지)
            bk_los = (bk_rn / len(df_paid_bk)) if len(df_paid_bk) > 0 else 0
            cn_los = (cn_rn / len(df_list_cn)) if len(df_list_cn) > 0 else 0
