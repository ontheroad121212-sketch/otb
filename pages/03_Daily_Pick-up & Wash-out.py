...
try:
    ...
    with st.sidebar:
        ...
        # [수정] 일자 선택 -> 달력 기간 선택 (date_input)
        # 날짜 범위 기본값 설정 (데이터가 있으면 데이터 범위, 없으면 오늘)
        ...
        if res_dates_all:
            try:
                # 가장 과거 데이터 ~ 가장 최근 데이터
                default_min = datetime.strptime(res_dates_all[-1], "%Y-%m-%d").date()
                default_max = datetime.strptime(res_dates_all[0], "%Y-%m-%d").date()
            except: pass
            
        # date_input 위젯 생성
        dates_selected = st.date_input(
            "기간 선택",
            value=(default_max, default_max), # 기본값: 가장 최근 날짜
            min_value=default_min - timedelta(days=365*2), # 여유있게
            max_value=datetime.now().date() + timedelta(days=365)
        )
        
        # date_input은 범위 선택 시 (start, end) 튜플을 반환하지만,
        # 선택 중일 땐 (start,) 만 반환할 수 있음
        sel_start_str, sel_end_str = None, None
        
        if isinstance(dates_selected, tuple):
            if len(dates_selected) > 0:
                sel_start_str = dates_selected[0].strftime('%Y-%m-%d')
            if len(dates_selected) > 1:
                sel_end_str = dates_selected[1].strftime('%Y-%m-%d')
            # 만약 하나만 선택됐다면 start = end 처리
            if sel_start_str and not sel_end_str:
                sel_end_str = sel_start_str
        
        st.divider()
        # [유지] OTB는 기존 selectbox 유지
    ...
    # 데이터 로드 (기간 필터링 적용)
    # 선택된 기간(Start ~ End)에 해당하는 모든 Snapshot_Date 데이터를 가져옴
    if sel_start_str and sel_end_str and not df_all.empty:
        # 예약리스트 출처 (Reservation 타입)
        # Snapshot_Date가 선택 기간 내에 있는 데이터만 필터링
        mask_bk = (df_all['Data_Type']=='Reservation') & \
                  (df_all['Snapshot_Date'] >= sel_start_str) & \
                  (df_all['Snapshot_Date'] <= sel_end_str)
    ...
    with tabs[0]:
        # 헤더에 조회 기간 표시
        disp_date = f"{sel_start_str} ~ {sel_end_str}" if sel_start_str else "기간 미선택"
