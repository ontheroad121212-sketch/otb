...
# ==============================================================================
# 1. 페이지 설정 및 CSS 스타일링 (대시보드 레이아웃 유지)
# ==============================================================================
st.set_page_config(
...
# ==============================================================================
# 4. 엑셀/CSV 파일 처리 및 매핑 로직 (조식 전수조사 및 OTB 마지막 셀 추출 이식)
# ==============================================================================
...
def process_data(file, status, force_otb=False):
    try:
        ...
        # ---------------------------------------------------------
        # Case A: OTB 데이터 처리 (마지막 셀 매출 추출 로직)
        # ---------------------------------------------------------
        if is_otb:
            ...
            for r in range(min(15, len(df_raw))):
                ...
                for v in row_vals:
                    ...
                    if match:
                        target_month_date = pd.to_datetime(match.group() + "-01"); break
                if target_month_date: break
        ...
        # ---------------------------------------------------------
        # Case B: 예약/취소 리스트 처리 (조식 전수조사 로직)
        # ---------------------------------------------------------
        else:
            header_idx = -1
            ...
            if header_idx != -1:
                ...
                def classify_nat(row):
                    ...
                    if any(x in str(row.get('Nat_Orig', '')).upper() for x in ['CHN', 'HKG', 'TWN']): return 'CHN'
                    return 'OTH'
...
def render_analysis_tab(target_df, title_prefix, unique_key, color_scale="Blues"):
    ...
    with t5:
        show_dataframe_with_style(add_total_row(target_df.groupby('Room_Type').agg({'RN': 'sum', 'Room_Revenue': 'sum', 'Total_Revenue': 'sum'}).reset_index(), 'Room_Type'))
    with t6:
    ...
    if selected_date and not df_all.empty:
        ...
        with main_tab0:
            ...
            if not df_paid_bk.empty:
                show_dataframe_with_style(add_total_row(df_paid_bk.groupby('Segment').agg({'RN': 'sum','Room_Revenue': 'sum'}).reset_index(), 'Segment')) 
        ...
        with main_tab5:
            ...
            if df_otb.empty: st.warning("⚠️ OTB 데이터가 없습니다.")
                ...
                grp_otb = df_otb.groupby('M').agg({'Room_Revenue':'sum'}).reset_index()
                ...
                fig = go.Figure()
                fig.add_trace(go.Bar(x=fin['Month_Label'], y=fin['Room_Revenue'], name='OTB (현재)', marker_color='#2E86C1', text=fin['Rate'].apply(lambda x: f"{x:.1f}%"), textposition='outside'))
                fig.add_trace(go.Scatter(x=fin['Month_Label'], y=fin['Budget'], name='Budget (목표)', line=dict(color='red', dash='dot', width=3)))
