...
# ==============================================================================
# 4. 엑셀 파일 처리 및 매핑 로직 (좌표 기반 정밀 수정)
# ==============================================================================
...
def process_data(uploaded_file, status, force_otb=False):
    """
    [사용자 요청 반영 핵심 로직]
    1. 조식: 예약/취소 리스트 K열(index 10)에서 'BF' 포함 여부 확인.
    2. OTB: S열(index 18)의 가장 마지막 행 값을 월 총액으로 추출.
    """
    try:
        ...
        if is_otb:
            # ------------------------------------
            # CASE A: Sales on the Book (OTB)
            # ------------------------------------
            # 월 식별: Column A(0)의 5행(index 4) 날짜 데이터 활용
            first_date_raw = df_raw.iloc[4, 0]
            month_dt = pd.to_datetime(first_date_raw, errors='coerce')
            
            # [수정] S열(index 18)의 가장 마지막 행 찾기
            # 유효한 행 중 가장 마지막 행 (보통 '소계' 행)
            last_row_idx = df_raw.iloc[4:, 0].last_valid_index()
            
            # 매출 데이터 추출 (S열 = index 18)
            revenue_val = str(df_raw.iloc[last_row_idx, 18])
            revenue_clean = pd.to_numeric(revenue_val.replace(',', '').replace('₩', '').replace(' ', ''), errors='coerce')
            
            # 객실수(RN) 추출 (보통 O열 = index 14 근처의 합계 객실수)
            # S열이 매출이라면 그 앞 열들 중 '객실수' 합계를 찾음
            rn_val = str(df_raw.iloc[last_row_idx, 14]) # 합계 객실수 열 추정 (Snippet 기준 14번)
            rn_clean = pd.to_numeric(rn_val.replace(',', ''), errors='coerce')
            ...
            # ------------------------------------
            # CASE B: 예약/취소 리스트 (조식 집계)
            # ------------------------------------
            # Header Row 3 (index 2) 적용
            ...
            # [핵심] 조식 식별 로직: 서비스코드(Service_Code)에 'BF'가 포함되어 있는지 확인
            def classify_breakfast(row):
                svc = str(row.get('Service_Code', '')).upper()
                # 공란이거나 nan이면 룸온리, BF 포함이면 조식포함
                if svc == 'NAN' or not svc.strip():
...
def render_tab(df, key_prefix):
    ...
    with t1:
        ...
        c1,c2=st.columns(2)
        c1.plotly_chart(px.pie(s, values='Room_Revenue', names='Segment', title="매출 비중"), use_container_width=True, key=f"{key_prefix}_seg_pie")
    ...
    with t4:
        df['LG'] = pd.cut(df['Lead_Time'], [-1,0,3,7,14,30,60,90,999], labels=['0일','1-3일','4-7일','8-14일','15-30일','31-60일','61-90일','90일+'])
    ...
    with t6:
        ...
        st.plotly_chart(px.bar(w, x='Day_Type', y='Room_Revenue', title="평일/주말 매출"), use_container_width=True, key=f"{key_prefix}_day_bar")
        show_dataframe_with_style(add_total_row(w, 'Day_Type'))
    with t7:
        n = df.groupby('Nat_Group').agg({'RN':'sum','Room_Revenue':'sum'}).reset_index()
        st.plotly_chart(px.pie(n, values='RN', names='Nat_Group', title="국적 비중"), use_container_width=True, key=f"{key_prefix}_nat_pie")
...
try:
    ...
    with st.sidebar:
        ...
        with st.expander("2. 취소 리스트", expanded=True):
            ...
            if f2 and st.button("취소 저장"):
                ...
                if not df.empty and save_to_firestore(df): st.success("저장 성공!"); time.sleep(1); st.cache_data.clear(); st.rerun()
        with st.expander("3. OTB (영업현황 12개월)", expanded=True):
            f3_list = st.file_uploader("OTB 파일들 (복수 선택)", type=['xlsx','csv'], key="f3", accept_multiple_files=True)
            if f3_list and st.button("OTB 통합 저장"):
                all_otb = [process_data(f, "Booked", force_otb=True) for f in f3_list]
                combined = pd.concat([d for d in all_otb if not d.empty], ignore_index=True)
                if not combined.empty and save_to_firestore(combined): st.success("OTB 통합 저장 완료!"); time.sleep(1); st.cache_data.clear(); st.rerun()
    ...
    if selected_date and not df_all.empty:
        ...
        with m0:
            ...
            c1.metric("예약 건수", f"{len(df_paid_bk):,.0f}")
            c2.metric("예약 RN", f"{df_paid_bk['RN'].sum():,.0f}")
            c3.metric("객실 매출", f"{df_paid_bk['Room_Revenue'].sum():,.0f}")
            c4.metric("취소 RN", f"{df_list_cn['RN'].sum():,.0f}")
            st.subheader("📊 세그먼트별 픽업")
        ...
        with m1: render_tab(df_paid_bk, "bk")
        with m2: render_tab(df_list_cn, "cn")
        with m3: render_tab(df_total_paid, "tot")
        with m4:
