...
def normalize_and_map_columns(df):
    ...
    rules = {
        ...
        # Lead_Time은 여기서 처리하지 않고 process_data에서 강제 주입함
    }
...
def process_data(uploaded_file, status, force_otb=False):
    try:
        ...
        # ---------------------------------------------------------
        # Case B: 예약/취소 (리드타임 선확보 & 후주입)
        # ---------------------------------------------------------
        else:
            # 1. 엑셀 3행(Index 2)을 헤더로 읽기
            if uploaded_file.name.endswith('.csv'):
                try: df_to_use = pd.read_csv(uploaded_file, header=2)
                except: df_to_use = pd.read_csv(uploaded_file, header=2, encoding='cp949')
            else:
                df_to_use = pd.read_excel(uploaded_file, header=2)

            # 2. [핵심] 컬럼명이 바뀌기 전에 리드타임 데이터 미리 확보 (백업)
            lead_time_backup = None
            for col in df_to_use.columns:
                col_str = str(col).upper().replace(" ", "").replace("_", "").replace("/", "")
                if "LEAD" in col_str or "리드" in col_str or "LT" in col_str:
                    lead_time_backup = df_to_use[col]
                    break
...
def show_dataframe_with_style(df):
    ...
    def highlight_total(row):
        return ['background-color: #fff9c4; font-weight: bold; color: black; border-top: 2px solid black'] * len(row) if any(str(val) == "TOTAL" for val in row) else [''] * len(row)
...
def render_analysis_tab(target_df, title_prefix, unique_key, color_scale="Blues"):
    ...
    with t8:
        ...
        if 'Breakfast' in target_df.columns:
            ...
            c1, c2 = st.columns(2)
            fig_bf_pie = px.pie(bf_stats, values='RN', names='Breakfast', title="조식 비중")
            c1.plotly_chart(fig_bf_pie, use_container_width=True, key=f"{unique_key}_bf_p")
            fig_bf_bar = px.bar(bf_stats, x='Breakfast', y='Room_Revenue', title="조식 매출")
            c2.plotly_chart(fig_bf_bar, use_container_width=True, key=f"{unique_key}_bf_b")
...
try:
    ...
    with st.sidebar:
        ...
        f3 = st.file_uploader("OTB (12개월 통합)", type=['xlsx','csv'], key="f3", accept_multiple_files=True)
    ...
    if selected_date and not df_all.empty:
        ...
        with tabs[0]:
            ...
            with c_l:
                if not df_paid_bk.empty: 
                    fig_gm_pie = px.pie(df_paid_bk.groupby('Nat_Group')['RN'].sum().reset_index(), values='RN', names='Nat_Group', hole=0.4, title="국적 비중")
            ...
            with c_r:
                ...
                if not comb_m.empty: 
                    fig_gm_bar = px.bar(comb_m, x='Stay_Month', y='RN', color='Type', barmode='group', title="예약/취소 추이")
        ...
        with tabs[4]: st.subheader("🆓 0원 예약"); st.dataframe(df_list[(df_list['Status'] == 'Booked') & (df_list['Total_Revenue'] <= 0)][['Guest_Name', 'CheckIn', 'Account', 'Room_Type']], use_container_width=True)
