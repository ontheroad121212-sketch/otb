...
# ==============================================================================
# 4. 엑셀/CSV 파일 처리 및 매핑 로직 (조식 전수조사 로직 포함)
# ==============================================================================
...
def process_data(file, status, force_otb=False):
    try:
        ...
        if header_idx != -1:
            ...
            # [수정] 조식 식별 로직: 특정 열만 보지 않고 모든 열을 다 뒤집니다.
            def check_breakfast_full_scan(row):
                # 해당 행의 모든 셀 값을 문자열로 합쳐서 'BF'가 있는지 확인
                combined_row_text = "".join(row.astype(str).values).upper()
                if 'BF' in combined_row_text:
                    return 'Included (조식포함)'
                return 'Not Included (불포함)'
            
            # 모든 열을 전수 조사하여 조식 데이터 생성
            breakfast_series = df_data.apply(check_breakfast_full_scan, axis=1)
            
            # 헤더 입히고 매핑
            df_data.columns = df_raw.iloc[header_idx].values
            df = normalize_and_map_columns(df_data).copy()
            df['Breakfast'] = breakfast_series 
    ...
    if selected_date and not df_all.empty:
        ...
        with main_tabs[5]:
            ...
            if df_otb.empty: st.warning("OTB 데이터 없음")
                ...
                st.plotly_chart(fig, use_container_width=True, key="otb_final_chart_integrity")
