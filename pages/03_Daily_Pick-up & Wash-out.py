...
# ==============================================================================
# 4. 파일 처리 (CSV 인코딩 해결 및 리드타임 강제 추출)
# ==============================================================================
...
def process_data(file, status, force_otb=False):
    try:
        ...
        # [CSV/Excel 읽기]
        # CSV 파일일 경우 cp949 인코딩을 사용하여 한글 헤더('리드타임') 깨짐 방지
        ...
        # [B] 예약/취소 리스트 처리
            ...
            # 1. 3행(Index 2)을 헤더로 사용하기 위해 데이터 분리
            header_row = df_raw.iloc[2].astype(str).tolist()
            df_data = df_raw.iloc[3:].reset_index(drop=True)
            df_data.columns = header_row
            
            # 2. [핵심] '리드타임' 열을 이름으로 찾아서 데이터 확보 (매핑 전)
            lead_time_series = 0
            for col in df_data.columns:
                if '리드' in str(col) or 'Lead' in str(col) or 'LT' in str(col):
                    lead_time_series = pd.to_numeric(df_data[col].astype(str).str.replace(',', ''), errors='coerce').fillna(0)
                    break
...
def render_tabs(df, key):
    ...
    with t6:
        ...
        c1, c2 = st.columns(2)
        c1.plotly_chart(px.bar(w, x='Day_Type', y='Room_Revenue'), use_container_width=True, key=f"{key}_wd_bar")
        c2.plotly_chart(px.pie(w, values='RN', names='Day_Type'), use_container_width=True, key=f"{key}_wd_pie")
    ...
    with t8: # 조식
        if 'Breakfast' in df.columns:
            ...
            c1, c2 = st.columns(2)
            c1.plotly_chart(px.pie(b, values='RN', names='Breakfast'), use_container_width=True, key=f"{key}_bf_pie")
...
try:
    ...
    with st.sidebar:
        ...
        f3 = st.file_uploader("OTB 파일", type=['xlsx','csv'], key="f3", accept_multiple_files=True)
        if f3 and st.button("OTB 저장"):
            otb_list = [process_data(f, "Booked", force_otb=True) for f in f3]
            if otb_list and save_to_firestore(pd.concat(otb_list, ignore_index=True)): st.cache_data.clear(); st.rerun()
    ...
    if sel_date and not df_all.empty:
        ...
        with tabs[0]: # GM 요약
            ...
            st.divider()
            style_df(add_total(df_bk.groupby('Segment').agg({'RN':'sum','Room_Revenue':'sum'}).reset_index(), 'Segment'))
