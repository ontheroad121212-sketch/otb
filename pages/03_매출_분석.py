    ...
    try:
        ...
        if is_otb:
            ...
            try:
                ...
                df['Total_Revenue'] = df['Room_Revenue'] # OTB는 보통 객실매출만 있으므로 동일하게 처리
            except:
...
def render_analysis_tab(target_df, title_prefix, color_scale="Blues"):
    ...
    with t1:
        ...
        # ADR 계산은 add_total_row가 아니라 여기서도 개별 행에 대해 해줘야 함
    ...
    if selected_date and not df_all.empty:
        ...
        if df.empty:
            ...
            with main_tab0:
                ...
                c6.metric("❌ 취소 건수", f"{cn_cnt:,.0f} 건")
                c7.metric("❌ 취소 RN", f"{cn_rn:,.0f} 박")
                c8.metric("❌ 취소 객실매출", f"{cn_room_rev:,.0f} 원")
                c9.metric("❌ 취소 총매출", f"{cn_total_rev:,.0f} 원")
                c10.metric("❌ 취소 객실ADR", f"{cn_adr_room:,.0f} 원")
