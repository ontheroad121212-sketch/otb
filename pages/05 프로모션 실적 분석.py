...
def prepare_df(raw_data):
    ...
    df['주말여부'] = df['입실일자'].dt.dayofweek >= 4 
        ...
        if not all_data:
            ...
            with col_a:
                ...
                if compare_on and compare_promo:
                    ...
                    st.plotly_chart(px.line(combined_p, x='예약일자', y='누적_RN', color='구분', title="누적 예약 생산 비교"), use_container_width=True)
                else:
                    st.plotly_chart(px.line(df_main_p, x='예약일자', y='누적_RN', title="누적 예약 생산 (Pace)"), use_container_width=True)
            with col_b:
            ...
            with col_c:
                ...
                if compare_on and compare_promo:
                    ...
                    st.plotly_chart(px.bar(combined_nat, x='proportion', y='국적', color='구분', barmode='group', orientation='h', title="국적 비중 비교"), use_container_width=True)
                else:
                    st.plotly_chart(px.pie(df_main, names='국적', hole=0.5, title="기준 국적 분포"), use_container_width=True)
            with col_d:
                ...
                if compare_on and compare_promo:
                    ...
                    st.plotly_chart(px.bar(combined_prod, x='상품구분', y='proportion', color='구분', barmode='group', title="조식 포함 비중 비교"), use_container_width=True)
                else:
                    st.plotly_chart(px.pie(df_main, names='상품구분', title="기준 상품군 비중"), use_container_width=True)
            ...
            if compare_on and compare_promo:
                ...
                with st.expander(f"📢 {target_promo['partner']} vs {compare_promo['partner']} 실적 총평", expanded=True):
                    ...
                    3. **패턴 인사이트:** - **리드타임:** {df_main['LT구간'].mode()[0] if not df_main['LT구간'].empty else '정보없음'} 비중이 가장 높습니다.
                        - **국적 변화:** {'국내 고객' if not nat_main.empty and nat_main.iloc[0]['국적'] == 'KOR' else '외국인 고객'} 비중이 가장 큰 기여를 하고 있습니다.
            ...
            # --- 상세 예약 패턴 분석 (기준 데이터 중심) ---
            st.divider()
            st.subheader("📊 상세 예약 패턴 분석 (기준 데이터 중심)")
            
            # [핀셋 조정: 요금타입 비중 및 상세 패턴]
            d1, d2, d3 = st.columns(3)
            with d1:
                st.markdown("#### 🔥 투숙 집중도 히트맵")
                ...
                st.plotly_chart(px.imshow(heat_data, text_auto=True, color_continuous_scale="YlOrRd"), use_container_width=True)
            
            with d2:
                st.markdown("#### 🎫 요금타입별 판매 비중 (Rate Mix)")
                if '요금타입' in df_main.columns:
                    rate_mix = df_main['요금타입'].value_counts().reset_index()
                    st.plotly_chart(px.pie(rate_mix, names='요금타입', values='count', hole=0.4, color_discrete_sequence=px.colors.qualitative.Set3), use_container_width=True)
                else:
                    st.warning("데이터에 '요금타입' 컬럼이 없습니다.")
            
            with d3:
                st.markdown("#### 🏨 객실 타입별 매출 합계")
                room_perf = df_main.groupby('객실타입')['총금액'].sum().reset_index()
                st.plotly_chart(px.bar(room_perf, x='객실타입', y='총금액', color='총금액', color_continuous_scale='Viridis'), use_container_width=True)
            ...
            with col_low1:
                ...
                st.plotly_chart(px.bar(lt_sum, x='LT구간', y='count', color='count', title="Lead Time Window"), use_container_width=True)
            with col_low2:
                st.markdown("#### 🍱 부대수익 서비스 분석")
                ...
                if all_svcs:
                    ...
                    st.plotly_chart(px.bar(svc_df, x='서비스명', y='건수', color='건수', title="Ancillary Revenue Analysis"), use_container_width=True)
            ...
            try:
                ...
                if st.button("🔥 Firestore 저장"):
                    ...
                    db.collection("promotions").document(doc_id).set({
                        "partner": val_partner, "start_date": start_date, "end_date": end_date, 
                        "upload_date": str(datetime.date.today()), "data": df_final.to_dict(orient='records')
                    })
