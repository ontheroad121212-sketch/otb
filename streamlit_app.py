...
<style>
    ...
    th { white-space: pre-wrap !important; text-align: center !important; vertical-align: bottom !important; }
</style>
...
st.caption("오늘 파일 8개(각 월별 어제/오늘)를 모두 업로드하면 자동으로 비교합니다.")
...
for i, tab in enumerate(tabs):
    ...
    with tab:
        ...
        # [로직 개선] 파일 이름으로 정렬하여 Yesterday / Today 구분
        if files:
            # 파일 이름 순으로 정렬 (예: 2026-01-20.xlsx -> 2026-01-21.xlsx)
            files.sort(key=lambda x: x['name'])
            
            if len(files) >= 2:
                # 뒤에 있는 파일이 최신(Today), 앞에 있는 파일이 과거(Yesterday)
                f_prev = files[-2] # 끝에서 두번째 (어제)
                f_curr = files[-1] # 맨 마지막 (오늘)
                
                df_curr, sob_curr = f_curr['data'], f_curr['sob']
                df_prev = f_prev['data']
                st.caption(f"🔥 파일 비교: {f_prev['name']} (Pre) vs {f_curr['name']} (Today)")
                
            else:
                # 파일 1개뿐이면 DB 비교
                df_curr, sob_curr = files[0]['data'], files[0]['sob']
                df_prev, _ = get_full_data_by_date(compare_date_str, current_month)
                st.caption(f"📂 파일 vs DB 비교 ({files[0]['name']} vs {compare_date_str})")
        else:
            # 파일 없으면 DB 조회
        ...
        if df_prev is not None:
            ...
            # [중요] Merge Key 타입을 문자열로 통일하여 오류 방지
        ...
        styler = styler.set_properties(subset=curr_cols, **{
            'font-weight': '700', 'font-size': '12px', 'border-left': '1px solid #cbd5e1', 'border-right': '1px solid #cbd5e1'
        })
        
        # 3. Var(변화) 그룹 - 노랑 파스텔 + 빨강/초록
