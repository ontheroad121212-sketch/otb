...
# ------------------------------------------------------------------
# 1. 페이지 설정 및 CSS 디자인 (Full Version)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 3. 데이터 처리 로직 (오류 수정: int/float 형변환 추가)
# ------------------------------------------------------------------
...
def find_header_and_process(file):
    try:
        ...
        # [오류 수정 핵심] numpy.int64 -> python int로 강제 변환
...
for i, tab in enumerate(tabs):
    ...
    with tab:
        ...
        # [로직] 파일 이름순 정렬을 통해 어제/오늘 구분
        if files:
            files.sort(key=lambda x: x['name']) # 이름 오름차순 정렬
            
            if len(files) >= 2:
                f_prev = files[-2] # 끝에서 두 번째 (어제)
