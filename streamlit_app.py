...
TRANS = {
    ...
    "zh": {
        "title": "酒店经营综合日报 (Daily Pace Report)", # 회장님이 볼 제목
        "summary_title": "月度经营业绩摘要 (Monthly Performance)", # 성과 요약
        "budget": "预算 (Budget)", 
        "actual": "实际营收 (Actual)", 
        "variance": "差异 (Var)",
        "segment": "客源细分 (Segment)", 
        "total": "总吉 (TOTAL)",
        "pre": "昨日 (Pre)", # 어제
        "today": "今日 (Today)", # 오늘
        "var": "增减 (Var)" # 변화량
...
for i, tab in enumerate(tabs):
    ...
    with tab:
        ...
        # HTML S.O.B 대시보드 렌더링
# ▼▼▼ [핀셋 수정 3] S.O.B 대시보드 (중국어 변수 적용) ▼▼▼
        st.markdown(f"""
        <div class="sob-container">
            <div class="sob-header">📊 {cur_m}월 {T['summary_title']}</div>
            <div class="sob-grid">
                <div>
