...
def run_forecasting():
    ...
    st.caption("고도화: 리드타임 감쇄 로직 반영 + 현재 추세 기반 달성 가시성 분석")
    ...
    # 5. [정밀 엔진 고도화] 리드타임 감쇄 및 추세 반영 로직
    # [A] 리드타임 감쇄(Time-decay): 입실 임박 시 픽업 강도가 둔화되는 현실적 현상 시뮬레이션
    # 남은 일수(rem_days)가 전체 판매 기간(auto_rem_days) 대비 줄어들수록 픽업 속도가 로그함수 형태로 감쇄
    time_decay_factor = np.log1p(rem_days) / np.log1p(auto_rem_days) if auto_rem_days > 0 else 1.0
    
    # 수정된 미래 픽업량 = 일평균 픽업 * 요일지수 * 가속도 * 리드타임(lt) 보정 * 감쇄(decay) 계수 * 남은 날짜
    lt_factor = (1.0 + (1.0 / np.log1p(rem_days)))
    expected_pickup_rms = actual_pace * auto_dow_index * accel * lt_factor * time_decay_factor * rem_days
    ...
    with s1:
        ...
        st.caption(f"리드타임 감쇄 계수: {time_decay_factor:.2f} 반영됨")
        
    with s2:
        st.write("**일평균 필요 예약량**")
        # 감쇄 계수가 반영된 현재의 실질적인 페이스와 비교
        effective_current_pace = actual_pace * time_decay_factor
        color = "normal" if effective_current_pace >= required_daily_rms else "inverse"
        st.metric("Required Pickup", f"{required_daily_rms:.1f} 박", f"{effective_current_pace - required_daily_rms:+.1f} (현재 대비)", delta_color=color)
    ...
    with tab2:
        ...
        daily_pace = expected_pickup_rms / max(1, rem_days)
        ...
        st.caption("💡 입실일이 가까울수록 픽업이 완만해지는 감쇄 로직이 적용된 곡선입니다.")
