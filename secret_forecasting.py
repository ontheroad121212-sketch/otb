...
def run_forecasting():
    st.title("🏛️ 총지배인(GM) 전략 의사결정 대시보드 v15.2")
    st.caption("최종 무삭제판: 3개년 비교 차트 + 미래 예약 곡선 + 운영 효율성 지표")
    ...
    # 5. [신규/보강] 시각화 대시보드 - 탭으로 구성하여 생략 방지
    st.divider()
    tab1, tab2 = st.tabs(["📊 3개년 성과 비교 (Bar Chart)", "🔮 미래 예측 곡선 (Booking Curve)"])
    
    with tab1:
        st.subheader("🏁 전년 실적 및 사업 계획 대비 예측 비교")
        # 막대 그래프 데이터
        ...
        st.caption("💡 막대 그래프를 통해 사업 계획 달성 가능성을 시각적으로 확인하세요.")
    ...
    with tab2:
        ...
        st.info(f"📈 오늘부터 남은 {rem_days}일 동안 일평균 {daily_pickup_avg:.1f}실씩 예약이 추가될 것으로 보입니다.")
    ...
    with col_left:
        ...
        report_df = pd.DataFrame({
            "구분": ["실질 가용 재고", "현재 확정(OTB)", "예상 추가 픽업", "이탈 예상(Wash)"],
    ...
    with col_right:
        ...
        if achieve_rate >= 100:
            ...
            st.write(f"목표 대비 {int(final_rev_man - budget_rev)}만원 초과 달성이 예상됩니다.")
        else:
            st.error(f"⚠️ **목표 미달 비상 ({achieve_rate:.1f}%)**")
            st.write(f"목표까지 {int(budget_rev - final_rev_man)}만원 부족합니다.")

    # 8. 수익성 및 인건비 효율 가이드 (무삭제)
    st.write("---")
